 ...
 * 
 * Filename: RecreateMatplotlibAnimationFrameWorker2.java
...
/**
 * The Class RecreateMatplotlibAnimationFrameWorker2.
 *
 * @author Eric J. Lingerfelt
 */
public class RecreateMatplotlibAnimationFrameWorker2 extends SwingWorker<ErrorResult, Void>{
	 ...
	 */
	public RecreateMatplotlibAnimationFrameWorker2(Frame parent, VizExplMoviePanel panel, VizSet vs, MatplotlibAnimation va, int frame){
		this.parent = parent;
		this.vs = vs;
		this.va = va;
		this.panel = panel;
	...
	protected void done(){
		try{
			...
			if(!result.isError()){
				...
				panel.replaceFrameFile(file, frame);
			}else{
				panel.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
