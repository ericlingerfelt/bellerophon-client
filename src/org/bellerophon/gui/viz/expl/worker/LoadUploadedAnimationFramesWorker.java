 ...
 * @author Eric J. Lingerfelt
 */
public class LoadUploadedAnimationFramesWorker extends SwingWorker<ErrorResult, Void>{

	private Frame frame;
	private UploadedAnimation ua;
	private VizSet vs;
	private LoadUploadedAnimationFramesListener listener;
	 ...
	 * @param a the a
	 */
	public LoadUploadedAnimationFramesWorker(Frame frame, VizSet vs, UploadedAnimation ua, LoadUploadedAnimationFramesListener listener){
		this.frame = frame;
		this.vs = vs;
		this.ua = ua;
	...
	protected void done(){
		try{
			if(!isCancelled()){
				...
				if(!result.isError()){
					listener.uploadedAnimationFramesLoaded();
				}else{
