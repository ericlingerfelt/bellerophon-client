 ...
 * 
 * Filename: GetUploadedAnimationsWorker.java
...
/**
 * The Class GetUploadedAnimationsWorker.
 *
 * @author Eric J. Lingerfelt
 */
public class GetUploadedAnimationsWorker extends SwingWorker<ErrorResult, Void>{
	 ...
	 */
	public GetUploadedAnimationsWorker(Frame frame, VizManPanel panel, DelayDialog dialog){
		this.frame = frame;
		this.panel = panel;
		this.dialog = dialog;
	}
	...
	protected ErrorResult doInBackground(){
		return WebServiceCom.getInstance().doWebServiceComCall(panel.getData().getVizSet(), Action.GET_UPLOADED_ANIMATIONS);
	}
	...
	protected void done(){
		try{
			...
			if(!result.isError()){
				panel.setUploadAnimationPanelState(true);
				panel.setCurrentState(VizManPanel.VizManMode.UPLOAD);
				dialog.close();
			}else{
				dialog.close();
